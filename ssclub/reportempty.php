		...
		error_reporting(0);
			...
			if($_GET['name'] || $_GET['prodcut'] ||  $_GET['from'] || $_GET['to'] || $_GET['type'] || $_GET['nic'] || $_GET['LicenseNo'] || $_GET['pmodel'] || $_GET['membership'] || $_GET['billno'])
			{
				...
				if($billno)
				{
						...
						$query=mysql_query("select * from `sales` where `Sid`='$sales_id' order by 'Date' ASC") or die(mysql_error());			
				}
				if($cname)
				{
					
					$query=mysql_query("select * from `sales` where `Cname` LIKE '$cname%' order by 'Date' ASC") or die(mysql_error());	
				}else
				if($nic)
				{
					$query=mysql_query("select * from `sales` where `NIC` LIKE '$nic%' order by 'Date' ASC") or die(mysql_error());
				}else
				if($LicenseNo)
				{
					$query=mysql_query("select * from `sales` where `LicenseNo` LIKE '$LicenseNo%' order by 'Date' ASC") or die(mysql_error());
				}else
				if($product)
				{
					$query=mysql_query("select * from `sales` where `Product`='$product' order by 'Date' ASC") or die(mysql_error());	
				}
				if($product && $type)
				{
					$query=mysql_query("select * from `sales` where `Product`='$product' AND `SubProduct`='$type' order by 'Date' ASC") or die();	
				}
				if($product && $type && $pmodel)
				{
					
					$query=mysql_query("select * from `sales` where `Product`='$product' AND `SubProduct`='$type' AND `pmodel`='$pmodel' order by 'Date' ASC") or die();	
			}
				
				if($from)
				{
					$query=mysql_query("select * from `sales` where `Date`='$from' order by 'Date' ASC") or die(mysql_error());	
				}
				
				if($to)
				{
					$query=mysql_query("select * from `sales` where `Date`='$to' order by 'Date' ASC") or die(mysql_error());	
				}
				
				if($from && $to)
				{
					$query=mysql_query("select * from `sales` where  `Date`>='$from' AND `Date`<='$to' order by 'Date' ASC") or die(mysql_error());	
				}
				if($membership)
				{
					$query=mysql_query("select * from `sales` where `membership`='$membership' order by 'Date' ASC") or die(mysql_error());	
				}
				if($membership && $from)
				{
					$query=mysql_query("select * from `sales` where `membership`='$membership' AND `Date`>='$from' order by 'Date' ASC") or die(mysql_error());	
				}
				if($membership && $to)
				{
					$query=mysql_query("select * from `sales` where `membership`='$membership' AND `Date`='$to' order by 'Date' ASC") or die(mysql_error());	
				}
				if($membership && $from && $to)
				{
					$query=mysql_query("select * from `sales` where `membership`='$membership' AND `Date`>='$from' AND `Date`<='$to' order by 'Date' ASC") or die(mysql_error());	
				}
				if($cname && $from)
				{
					$query=mysql_query("select * from `sales` where `Cname`='$cname' AND `Date`>='$from' order by 'Date' ASC") or die(mysql_error());	
				}
				if($cname && $to)
				{
					$query=mysql_query("select * from `sales` where `Cname`='$cname' AND `Date`='$to' order by 'Date' ASC") or die(mysql_error());	
				}
				if($cname && $from && $to)
				{
					$query=mysql_query("select * from `sales` where `Cname`='$cname' AND `Date`>='$from' AND `Date`<='$to' order by 'Date' ASC") or die(mysql_error());	
				}
				if($product && $from)
				{
					$query=mysql_query("select * from `sales` where `Product`='$product' AND `Date`>='$from' order by 'Date' ASC") or die(mysql_error());	
				}
				if($product && $to)
				{
					$query=mysql_query("select * from `sales` where `Product`='$product' AND `Date`='$to' order by 'Date' ASC") or die(mysql_error());	
				}
				if($product && $from && $to)
				{
					$query=mysql_query("select * from `sales` where `Product`='$product' AND `Date`>='$from' AND `Date`<='$to' order by 'Date' ASC") or die(mysql_error());	
				}
				if($product && $type && $pmodel && $from && $to)
				{
					$query=mysql_query("select * from `sales` where `Product`='$product' AND `SubProduct`='$type' AND `pmodel`='$pmodel' AND `Date`>='$from' AND `Date`<='$to' order by 'Date' ASC") or die(mysql_error());	
				}
				if($product && $type && $pmodel && $from)
				{
					$query=mysql_query("select * from `sales` where `Product`='$product' AND `SubProduct`='$type' AND `pmodel`='$pmodel' AND `Date`>='$from' order by 'Date' ASC") or die(mysql_error());	
				}
				if($product && $type && $pmodel && $to)
				{
					$query=mysql_query("select * from `sales` where `Product`='$product' AND `SubProduct`='$type' AND `pmodel`='$pmodel' AND `Date`='$to' order by 'Date' ASC") or die(mysql_error());	
				}
				if($LicenseNo && $to)
				{
					$query=mysql_query("select * from `sales` where `LicenseNo`='$LicenseNo' AND `Date`='$to' order by 'Date' ASC") or die(mysql_error());
				}
				if($nic && $to)
				{
					$query=mysql_query("select * from `sales` where `NIC`='$nic' AND `Date`='$to' order by 'Date' ASC") or die(mysql_error());
				}
...
<table width="1280" border="1" align="center" style="color:#000;">
  ...
  <tr style="background:#CCC">
    ...
    <th width="60" scope="col" nowrap="nowrap">Weapon No</th>
 ...
 ?>
  ...
  <tr align="center">
    ...
    <td><?php echo $s_row['LicenseNo'];?></td>
     <td width="60"><?php echo $s_row1['weapon_no'];?></td>
  ...
  <tr align="center">
    ...
    <td><?php echo $qyrow['LicenseNo'];?></td>
     <td width="60"><?php echo $billrow['weapon_no'];?></td>
...
<!---For empty search reuslt ---->
					<?php
								...
								if(empty($_GET['name']) && empty($_GET['prodcut']) &&  empty($_GET['from']) && empty($_GET['to']) && empty($_GET['type']) && empty($_GET['nic']) && empty($_GET['LicenseNo']) && empty($_GET['membership']) && empty($_GET['billno']))
								{
									...
									if($_GET['bymodel'])
									{
										$qry=mysql_query("select * from `sales` ORDER BY `pmodel` ".$_GET['bymodel']."") or die();
									}
...
<table class="responstable">
    <tr class="sticky">
    <th>SNO</th>
    <th><?php if($_GET['by']==""){?><a style="color:white; href="store.php?page=reportempty&by=asc">Bill#</a><?php }?><?php if(isset($_GET['by'])=='asc'){?><a href="store.php?page=reportempty&by=desc">Bill#</a><?php }?></th>
    <th>Name</th>
    <th>Product</th>
    <th>Product Type</th>
    <th>
        <?php if($_GET['bymodel']==""){?><a style="color:white; href="store.php?page=reportempty&bymodel=asc">Product Model</a><?php }?>
        <?php if(isset($_GET['bymodel'])=="asc"){?> <a style="color:white; href="store.php?page=reportempty&bymodel=desc">Product Model</a><?php }?>
    </th>
    <th>License NO</th>
    <th>Weapon No</th>
    <th>Qty</th>
  ...
  <tr>
    ...
    <td width="60"><?php echo $row3['weapon_no'];?></td>
...
<table style="width: 400px;margin: 0 auto;" class="responstable">
  ...
  <tr>
    <td><?php echo $qty+$qty1;?></td>
    <td><?php echo $rate+$rate1;?></td>
    <td><?php echo $total1+$total2;?></td>
    <td><?php echo $tpurchase_price+$t_price;?></td>
  </tr>
