...
font-size: 17px;
color: black;">GST REG NO. 05-01-9801-170-73</span><span style="    margin-left: 646px;
    font-size: 17px;
    color: #000;">NTN NO : 2565606-6</span>
</div>
...
<th width="396" scope="col">Weapon No</th>
...
<td align="center"><?php if(isset($sid)){ echo $fetch['weapon_no'];}?></td>
...
<td align="center"><?php if(isset($sid)){ echo $record1['weapon_no'];}?></td>
...
ORDER BY `tax_id` DESC LIMIT 1";
      ...
      <tr>
		...
		<span id="get_grand_total"><?php if($g_total!=""){ echo $g_total; }else{echo $grandtotal;}?></span>
    ...
    <script>
		$(document).ready(function(){
			var grand_total = <?php echo $grandtotal?>;
			
			grand_total_field = $('#grand_total_field').attr('value',grand_total);
			grand_total_field = $(grand_total_field).val();
			
			$('#rectification').keyup(function(){
				rectification_figure = $(this).val();
				
					var remaining = parseInt(grand_total_field) + parseInt(rectification_figure);
					$('#grand_total_field').removeAttr('value');
					if(remaining > 0){
						
					 $('#get_grand_total').css({'font-size':'100%','color':'#000000'}).text(remaining);
					}else{
						$('#get_grand_total').css({'font-size':'9px','color':'red'}).text('You are trying to enter invalid figure');
					} 
					
					if($('#grand_total_field').val() == 'NaN'){
						$('#grand_total_field').attr('value','');
					}
			});
