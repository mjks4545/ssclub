...
.icon-btn i {
	...
	<?php 
        ...
        $done['16']=16;
					...
					$q3=mysql_query("SELECT BoothNO,Reg_NO,TimeOut,Name FROM checkin WHERE TimeOut='' ORDER BY Reg_NO DESC");
	...
	//echo '<div class="square-state">';
		...
		//print_r($done);
								...
								$booths[$row['BoothNO']] = '<span class="badge badge-important" style="width:5%;" ><a class="icon-btn" style=" background-image:-moz-linear-gradient(center top , #ddd, #fffdfe);" href="#" ><i class="icon-group">' . $row['BoothNO'] . '</i><div>' . $row['BoothNO'] . '</div></span></a>';
									...
									$booths[$row['BoothNO']] = '<span class="badge badge-success" style="padding:7px;margin: 3%"><a id="btn-success" style="-moz-linear-gradient(center top , #2980B9, #2980B9)" class="icon-btn" href="http://localhost/SSClub/index.php?page=booth_res&B_No=' . $row['BoothNO'] . '" ><i class="icon-group">' . $row['Name'] . '</i></span></a>';
								 $count++;
