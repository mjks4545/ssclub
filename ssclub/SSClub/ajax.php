...
<?php
		if(isset($_GET['id']))
			{
				$product=$_GET['id'];
				$qry=mysql_query("select `p_type` from `product` where `p_name`='$product' GROUP BY `p_type`");	
?>
 ...
 <?php }?>
 	...
 	<?php
			...
			{
				$weapon=$_GET['weapon'];
				$qry=mysql_query("select `prd_type`,`pmodel` from `purchase` where `weapone_no`='$weapon' AND `product`='Fire Arms' GROUP BY `prd_type`");	
?>
